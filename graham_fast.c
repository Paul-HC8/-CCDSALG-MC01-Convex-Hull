/*
    ...
    There is no main() here; the driver test_fast.c supplies main().
...
/*
    ...
    d. Purpose      : compute the convex hull of pts[0..n-1] with Graham's Scan, using
                      the FAST (Quick) sort. Steps: pick the anchor (lowest y, then
...
*/
int grahamScanFast(Point *pts, int n, Point *hull)
{
    ...
    /* 2. Sort the remaining points by polar angle about the anchor (Quick sort). */
    quickSort(pts + 1, 0, n - 2, anchor);
    ...
    printf("[FAST / Quick sort] elapsed time: %.3f ms\n", elapsedMs);
